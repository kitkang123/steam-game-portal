...
const articleSchema = {
  ...
  properties: {
    ...
    summaryArticle: {
      type: 'STRING',
      description: '一篇完整的300字左右的中文游戏攻略短文，融合成成熟流畅的段落，包含上述的核心亮点、新手避坑指南和引导折扣结尾（字数必须控制在280-320字之间，中文）'
    }
  },
...
5. 必须输出符合 Schema 结构的数据。`;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(`[Gemini API] 调用第 ${attempt} 次失败: ${game.title} - ${error.message}`);
      if (attempt === maxRetries) {
        console.error(`[Gemini API] 连续 ${maxRetries} 次调用出错，停止生成。`);
        throw error;
      }
      console.log(`[Gemini API] 将在 ${retryDelay / 1000} 秒后重新尝试...`);
      await new Promise(resolve => setTimeout(resolve, retryDelay));
      retryDelay *= 2; // 指数递增等待时间
    }
  }
}
...
async function run() {
  try {
    ...
    if (existingIndex !== -1) {
      ...
      history.unshift(weeklyData); // 插入最前面
    }
